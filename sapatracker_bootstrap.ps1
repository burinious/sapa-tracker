...
New-Item -ItemType Directory -Force -Path "src\context","src\components","src\pages","src\utils","src\styles","src\constants" | Out-Null

npm i jspdf jspdf-autotable react-toastify
...
"@ | Set-Content -Encoding UTF8 "src\styles\app.css"

@"
/* src/constants/ideas.js */
export const SAPA_IDEAS = [
  ...
  "Daily Spend Leak Detector: recurring small drains (snacks, data, transport)",
  "Sapa Forecast: estimate days until cash runs low based on recent spending",
  "Soft Limits: set category soft limits; warn without shaming",
  "Goals: savings goals + safe-to-spend number",
  "Bills Calendar: rent, power, internet  due dates & postponement logic",
  "Offline-first: works without network; sync when cloud enabled",
  "AI Chat: ChatGPT-style responses summarizing spending, suggesting adjustments, generating PDFs"
];
"@ | Set-Content -Encoding UTF8 "src\constants\ideas.js"
...
"@ | Set-Content -Encoding UTF8 "src\context\AuthContext.jsx"
...
"@ | Set-Content -Encoding UTF8 "src\components\BillingNotice.jsx"
...
"@ | Set-Content -Encoding UTF8 "src\components\ProtectedRoute.jsx"
...
"@ | Set-Content -Encoding UTF8 "src\utils\localStore.js"
...
"@ | Set-Content -Encoding UTF8 "src\utils\localFinance.js"
...
"@ | Set-Content -Encoding UTF8 "src\utils\localSubscriptions.js"
...
"@ | Set-Content -Encoding UTF8 "src\utils\localShopping.js"
  ...
  return (
    <div style={{ display:"grid", gap:14 }}>
      ...
      <div className="card">
        ...
        {top ? <div><b>{top.name}</b>  {formatMoney(top.amount)}</div> : <div className="small">No expenses yet.</div>}
      </div>
    </div>
  );
}
...
"@ | Set-Content -Encoding UTF8 "src\pages\AddTransaction.jsx"
...
"@ | Set-Content -Encoding UTF8 "src\pages\Transactions.jsx"
...
export default function HouseShopping() {
  ...
  function exportPDF() {
    const doc = new jsPDF();
    const title = profile?.storeName ? `${profile.storeName}  House Shopping` : "Sapa Tracker  House Shopping";
    doc.setFontSize(16);
    doc.text(title, 14, 16);
    doc.setFontSize(10);
    doc.text(`Date: ${new Date().toLocaleString()}`, 14, 22);
    doc.text(`Total: ${Number(total).toLocaleString("en-NG")}`, 14, 28);
    ...
    const rows = items
      ...
      .map(i => [
        ...
        i.name,
        String(i.qty),
        `${Number(i.price||0).toLocaleString("en-NG")}`,
        `${(Number(i.price||0)*Number(i.qty||1)).toLocaleString("en-NG")}`,
      ]);

    autoTable(doc, {
      startY: 34,
      head: [["Category", "Item", "Qty", "Unit", "Total"]],
      body: rows,
    });

    doc.save("house-shopping.pdf");
  }
  ...
  return (
    <div style={{ display:"grid", gap:12 }}>
      ...
      <div className="card">
        <div className="row" style={{ justifyContent:"space-between" }}>
          <div style={{ fontWeight:800 }}>Add item</div>
          <button className="btn" type="button" onClick={exportPDF} disabled={items.length===0}>
            Export PDF
          </button>
        </div>

        <form onSubmit={add} style={{ display:"grid", gap:10, marginTop:10 }}>
      ...
      {items.length === 0 ? (
        <div className="card"><div className="small">No shopping items yet.</div></div>
      ) : (
        items.map((x) => (
...
"@ | Set-Content -Encoding UTF8 "src\pages\HouseShopping.jsx"
...
function nextDueDate(dueDay) {
  ...
  const candidate = new Date(y, m, Math.min(28, Number(dueDay || 1)), 12, 0, 0);
  if (candidate >= now) return candidate;
  return new Date(y, m + 1, Math.min(28, Number(dueDay || 1)), 12, 0, 0);
}
...
export default function Subscriptions() {
  ...
  function pay(id) { updateSubscription(user.uid, id, { status: "paid" }); setTick(t=>t+1); }
  function postpone(id) { updateSubscription(user.uid, id, { status: "postponed" }); setTick(t=>t+1); }
    ...
    <div style={{ display:"grid", gap:12 }}>
      ...
      <div className="card">
        ...
        <form onSubmit={add} style={{ display:"grid", gap:10 }}>
          ...
          <div className="small">AI will later suggest Pay now vs postpone based on cash reality.</div>
        </form>
      </div>
      ...
      {list.length === 0 ? (
        ...
        list.map((s) => {
          ...
          const dueText = days <= 0 ? "Due today" : `Due in ${days} day(s)`;
          return (
            <div key={s.id} className="card">
              ...
              <div className="small" style={{ marginTop:6 }}>
                Due day: {s.dueDay}  <b>{dueText}</b>  Status: <b>{s.status}</b>
              </div>
...
"@ | Set-Content -Encoding UTF8 "src\pages\Subscriptions.jsx"
...
"@ | Set-Content -Encoding UTF8 "src\pages\EditProfile.jsx"
...
export default function SapaAI() {
  ...
  const [messages, setMessages] = useState([
    { role: "assistant", content: "Im Sapa Tracker AI. Ask me about spending habits, subscriptions, or your shopping list." }
  ]);
  ...
  function respond() {
    ...
    const reply =
      `Heres what I see (local mode):\n` +
      ` Cash at hand: ${cash.toLocaleString("en-NG")}\n` +
      ` Transactions recorded: ${txCount}\n\n` +
      `Next: when we connect real AI, Ill give ChatGPT-style advice with your data.`;
  ...
  return (
    <div style={{ display:"grid", gap:12 }}>
      ...
      <div className="card">
        <div className="row">
          <input className="input" value={q} onChange={(e)=>setQ(e.target.value)} placeholder="Ask: should I postpone Netflix?" />
...
"@ | Set-Content -Encoding UTF8 "src\pages\SapaAI.jsx"
